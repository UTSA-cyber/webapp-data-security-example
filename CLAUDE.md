...
**Phases 1, 2, 3, and 4 complete.** What's wired up:
...
- **Phase 3 (frontend)** — Supabase JS client + TanStack Query + AuthProvider context + ToastProvider. Email/password login wired to GoTrue. **Single Dashboard route** (`/`) renders a role-specific view based on `activeRole` — there are *no* per-role URLs. The role switcher in the header is the only way to change views; the swap happens in place when the JWT updates. The switcher dropdown shows **all four roles** with three states: active (badge), held membership (clickable), no membership (greyed + "no access"). That's where the "you don't have administrator access" lesson lives. Per-role views (`frontend/src/views/*View.tsx`) display lists scoped by RLS. Invalid-view panes on Student and Instructor views compare user-visible row counts to `admin_row_count` for the same table, making silent SELECT denials pedagogically visible. The login page also exposes a "Sign in with SSO (Authentik)" button via `signInWithOAuth({ provider: 'keycloak' })`.
- **Phase 4 (deployment)** — `deploy/` holds a working docker-compose for the full self-hosted stack (Postgres + GoTrue + migrator sidecar + PostgREST + nginx-served Vite app + Authentik server/worker/db/redis), Nomad jobspecs mirroring it (`deploy/nomad/{supabase,app,authentik}.nomad.hcl`), and Vault policies + KV layout docs (`deploy/vault/`). The compose stack has been smoke-tested end to end: login as a seeded user, query classrooms (RLS filters to enrollment), query sites (empty — invalid view), and `admin_row_count` returns the global count for the diff UI. nginx in the app container reverse-proxies `/auth/v1` to GoTrue and `/rest/v1` to PostgREST so the frontend talks to one origin.
